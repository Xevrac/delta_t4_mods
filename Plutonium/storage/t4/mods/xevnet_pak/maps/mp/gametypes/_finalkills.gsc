...
CreateFKMenu( victim , attacker)
{
    ...
    self.bottom_fk_shader setShader("white",640,56);
    ...
    self.credits setText("^5Ported by^0: ^7Xevnet ^5Credits^0: ^7Horizonz^0, ^7Xevrac^0, ^7FzBr.d4rk");
...
onPlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration)
{
	if(isdefined(attacker) && isplayer(attacker))
	{
		if(attacker != self)
		{
			...
			level.doFK[attacker.team] = true;
			
			if(level.teamBased)
			{
				level.KillInfo[attacker.team]["attacker"] = attacker;
				level.KillInfo[attacker.team]["attackerNumber"] = attacker getEntityNumber();
				level.KillInfo[attacker.team]["victim"] = self;
				level.KillInfo[attacker.team]["deathTime"] = GetTime() / 1000;
			}
			else
			{
			...
			setdvar("final_distance", int(distance(self.origin, attacker.origin) * 0.0254));
		}
	}
}
...
endGame( winner, endReasonText )
{
	...
	maps\mp\gametypes\_vote::BeginVoteForMatch();
